...
def draw_field(img, corners, ids):  # only when 4 ID's are available
    if len(corners) == 4:
        ids_sorted = sorted(ids)  # Sort IDs in ascending order
        markers_sorted = [0] * 4  # Initialize list to hold sorted markers
        for i, id_number in enumerate(ids_sorted):
            index = ids.index(id_number)
            markers_sorted[i] = corners[index]
        contours = np.array(markers_sorted)

        # Reshape contours array to the correct format
        ...
        overlay = img.copy()
        cv2.fillPoly(overlay, [contours], color=(255, 215, 0))  # Pass contours as a list
