...
# DH Parameters
DH_params = [
    {'alpha': 1.57, 'a': 42.00, 'd': 46.50, 'theta_offset': 0.10},
    {'alpha': 0.00, 'a': 95.00, 'd': 0.00, 'theta_offset': 1.70},
    {'alpha': 0.00, 'a': 110.00, 'd': 2.00, 'theta_offset': 3.14}
]

BASE_HEIGHT = 150  # Base height in mm
    ...
    while True:
        ...
        if choice == '1':
            ...
            try:
                ...
                print(
                    f"Calculated theta angles (degrees): Base: {base_rad * RAD_TO_DEG}, Shoulder: {shoulder_rad * RAD_TO_DEG}, Elbow: {elbow_rad * RAD_TO_DEG}")
                print(
                    f"Calculated theta angles with offsets (degrees): Base: {base_offset * RAD_TO_DEG}, Shoulder: {shoulder_offset * RAD_TO_DEG}, Elbow: {elbow_offset * RAD_TO_DEG}")
                print(
                    f"Calculated theta angles with base offset of PI (degrees): Base: {base_pi_offset * RAD_TO_DEG}, Shoulder: {shoulder_pi_offset * RAD_TO_DEG}, Elbow: {elbow_pi_offset * RAD_TO_DEG}")
                print(
                    f"Calculated theta angles with base offset of PI and additional offsets (degrees): Base: {base_pi_offset_with_additional * RAD_TO_DEG}, Shoulder: {shoulder_offset * RAD_TO_DEG}, Elbow: {elbow_offset * RAD_TO_DEG}")
